import assert from "node:assert/strict";
import fs from "node:fs";
import path from "node:path";
import test from "node:test";

const rootDir = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..");

function readRequiredFile(relativePath) {
  const absolutePath = path.join(rootDir, relativePath);
  assert.equal(fs.existsSync(absolutePath), true, `Expected ${relativePath} to exist`);
  return fs.readFileSync(absolutePath, "utf8");
}

function assertContainsAll(source, values) {
  for (const value of values) {
    assert.match(source, new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  }
}

test("blacklist LLM markdown guide contains blacklist endpoint implementation context", () => {
  const markdown = readRequiredFile("public/llms/blacklist.md");

  assertContainsAll(markdown, [
    "# Whats91 Blacklist API LLM Guide",
    "https://graph.whats91.com/api/v2/message-blacklist",
    "GET /api/v2/message-blacklist",
    "GET /api/v2/message-blacklist/{blacklistUid}",
    "POST /api/v2/message-blacklist",
    "POST /api/v2/message-blacklist/{blacklistUid}",
    "DELETE /api/v2/message-blacklist/{blacklistUid}",
    "Authorization: Bearer",
    "senderId",
    "blacklistUid",
    "normalizedPhone",
    "displayPhone",
    "status",
    "ACTIVE",
    "INACTIVE",
    "Node.js",
    "PHP",
    "BLACKLIST_ENTRY_NOT_FOUND",
    "SENDER_NOT_ALLOWED",
    "VALIDATION_FAILED",
    "Debugging Checklist",
    "metadata.requestId",
  ]);
});

test("blacklist LLM copy registry maps the blacklist category to the public markdown guide", () => {
  const registry = readRequiredFile("src/lib/llm-copy-docs.ts");

  assertContainsAll(registry, [
    "blacklist",
    "/llms/blacklist.md",
    "Blacklist API LLM Guide",
    "getLlmCopyDocForCategory",
  ]);
});

test("llms.txt route discovers registered blacklist LLM guide", () => {
  const llmsRoute = readRequiredFile("src/app/llms.txt/route.ts");

  assertContainsAll(llmsRoute, [
    "getLlmCopyDocs()",
    "Copy-ready Markdown for LLM implementation and debugging context.",
  ]);
});
